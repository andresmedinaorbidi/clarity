...
export default function GoalCard({
  ...
  const generatedVisuals = useMemo(() => {
    if (!goalValue || goalVisual) return null;
    return getFieldVisuals("goal", goalValue, state, goalVisuals);
  }, [goalValue, goalVisual, state]);

  const displayValue = goalValue ? (
    goalVisual ? (
      ...
      renderCustomFieldDisplay(goalValue, generatedVisuals, goalLabel)
    ) : (
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <div 
          ...
          >
            <Target size={24} style={{ color: "#6B7280" }} />
          </div>
          <div>
            <p className="text-base font-semibold text-text-primary">{goalLabel}</p>
            <p className="text-sm text-text-muted">Custom goal</p>
          </div>
        </div>
      </div>
    )
